...
pub fn format_expr(out: &mut String, e: &Expr, parent_prec: u8) {
    let prec = expr_prec(e);
    let needs_parens = prec < parent_prec;
    if needs_parens {
        out.push('(');
    }
    format_expr_inner(out, e);
    if needs_parens {
        out.push(')');
    }
}
...
fn format_expr_inner(out: &mut String, e: &Expr) {
    match e {
        ...
        Expr::Neg { inner, .. } => {
            out.push('-');
            format_expr(out, inner, 7);
        }
        Expr::Not { inner, .. } => {
            out.push_str("not ");
            format_expr(out, inner, 3);
        }
        Expr::Binary { op, left, right, .. } => {
            let p = expr_prec(e);
            format_expr(out, left, p);
            out.push(' ');
            out.push_str(binop_text(*op));
            out.push(' ');
            format_expr(out, right, p + 1);
        }
        ...
        Expr::PairLit { first, second, .. } => {
            ...
            format_expr(out, first, 0);
            out.push_str(" and ");
            format_expr(out, second, 0);
        }
...
fn format_call(out: &mut String, c: &CallExpr) {
    out.push_str(&c.callee.display());
    if let Some(first) = &c.first {
        out.push(' ');
        format_expr(out, &first.expr, 0);
    }
